...
contract AnthillTestMain is Test {
    ...
    /////////// basic dag tests
        function testFindSentDagVote() public {
            (, bool voted, uint256 dist, uint256 depth, uint256 votePos, ) = anthill.findSentDagVote(address(8),address(5));
            assertEq(votePos, 0);
            assertEq(dist, 2);
            assertEq(depth, 1);
            assertEq(voted, true);
    ...
    ////////// Dag internal tests
        ...
        /////////// single votes
            function testChangeDistDepthRec() public {
                (, bool voted, uint256 rdist, uint256 depth, uint256 votePos, DagVote memory rDagVote) = anthill.findRecDagVote(address(16), address(4));
                assertEq(voted, true);
                assertEq(rdist, 0);
                assertEq(depth, 2);
                assertEq(votePos, 0);
        ...
        /////////// cells
            ...
            function testChangeDistDepthFromRecCellOnOp() public {
                ...
                uint256 count = anthill.readRecDagVoteCount(address(4), 1, 2);
                assertEq(count, 4);
                ...
                for (uint256 i = 0; i < anthill.readRecDagVoteCount(address(4), 0, 2); i++){
                ...
                for (uint256 i = anthill.readRecDagVoteCount(address(4), 0, 2)  ; i >= 1; i--){
        ...
        /////////// collapse and sort
            ...
            function testCollapseSortRec() public {
                ...
                (bool voted, uint256 votepos, DagVote memory rDagVote) = anthill.findRecDagVotePosAtDistDepth(address(18), address(8), 2, 1);
                assert (voted);
                (bool svoted, uint256 svotepos, DagVote memory sDagVote) =anthill.findSentDagVotePosAtDistDepth(address(18), address(8), 3, 1 );
                assert (svoted);
        ...
        /////////// combined square handlers
            ...
            function testHandleDagVoteMoveRise() public {
                ...
                // (bool voted2, uint256 votePos2, DagVote memory rDagVote) = anthill.findRecDagVotePosAtDistDepth(address(8), address(4), 0, 2);
                // assert (voted2);
    ...
    // ///////////// utils 
        ...
        function dagConsistencyCheckFrom(address voter) public {
            ...
            for (uint256 dist = 0; dist < 7; dist++){
                for (uint256 depth = 0; depth <= dist; depth++){
                    for (uint256 i = 0; i < anthill.readSentDagVoteCount(voter, dist, depth); i++){
                        DagVote memory sDagVote = anthill.readSentDagVote(voter, dist, depth, i);
                        DagVote memory rDagVote = anthill.readRecDagVote(sDagVote.id, dist-depth, depth, sDagVote.posInOther);
                        (bool isLocal, uint256 recordedDist, uint256 recordedDepth) = anthill.findSDistDepth(voter, sDagVote.id);
                        assert( isLocal);
                        // console.log("id", voter, rDagVote.id, sDagVote.id);
                        assertEq(rDagVote.id, voter);
                        assertEq(rDagVote.weight, sDagVote.weight);
                    ...
                    for (uint256 i = 0; i < anthill.readRecDagVoteCount(voter, dist-depth, depth); i++){
                        ...
                        (bool isLocal, uint256 recordedDist, uint256 recordedDepth) = anthill.findSDistDepth(rDagVote.id, voter);
                        assert( isLocal);
