...
contract AnthillTestMain is Test {
    ...
    function testFindSentDagVote() public {
        (, bool voted, uint256 dist, uint256 depth, uint256 votePos, ) = anthill.findSentDagVote(
            address(8),
            address(5)
        );
        assertEq(votePos, 0);
    ...
    function testChangeDistDepthRec() public {
        ...
        anthill.changeDistDepthSent(
            ...
            2,
            address(4),
            anthill.readRecDagVoteCount(address(4), 1, 2) - 1,
            rDagVote.weight
        );
        count = anthill.readRecDagVoteCount(address(4), 1, 2);
        assertEq(count, 5);
    ...
    function testCollapseSortRec() public {
        ...
        (bool voted, uint256 votepos, DagVote memory rDagVote) = anthill.findRecDagVotePosAtDistDepth(
            address(18),
            address(8),
            2,
            1
        ...
        assert(voted);
        (bool svoted, uint256 svotepos, DagVote memory sDagVote) = anthill.findSentDagVotePosAtDistDepth(
            address(18),
            address(8),
            3,
            1
        ...
        for (uint256 dist = 0; dist < 7; dist++) {
            for (uint256 depth = 0; depth <= dist; depth++) {
                for (uint256 i = 0; i < anthill.readSentDagVoteCount(voter, dist, depth); i++) {
                    ...
                    (bool isLocal, uint256 recordedDist, uint256 recordedDepth) = anthill.findSDistDepth(
                        voter,
                        sDagVote.id
                    );
                    assert(isLocal);
