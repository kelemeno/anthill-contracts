...
contract Anthill2 {
    ...
    //// Local tree finders
        ...
        function findRelDepthInner(address voter, address recipient) public view returns (bool isLocal, uint256 relRootDiff, uint256 rDist){
        ...
        function findDistancesRecNotLower(address voter, address recipient) public view returns (bool isLocal, uint256 sDist, uint256 rDist){
            if (treeVote[voter] == address(0) || treeVote[recipient] == address(0)) {
                return (false, 0, 0);
            }
        ...
        function findDistances(address voter, address recipient) public view returns (bool isLocal, uint256 sDist, uint256 rDist){
            if (treeVote[voter] == address(0) || treeVote[recipient] == address(0)) {
                return (false, 0, 0);
            }
    ...
    //// DAG finders
        ...
        function findSentDagVote(address voter, address recipient) public view returns (bool votable, bool voted, uint256 sDist,  uint256 rDist, uint256 votePos, DagVote memory dagVote){
        ...
        function findSentDagVoteNew(address voter, address recipient) public view returns (bool votable, bool voted, uint256 votePos, DagVote memory dagVote){
            (votable, voted, ,,votePos, dagVote) = findSentDagVote(voter, recipient);
        }

        function findRecDagVote(address voter, address recipient) public view returns (bool votable, bool voted, uint256 sdist,  uint256 depth, uint256 votePos, DagVote memory dagVote){
            bool isLocal;
            uint256 sDist;
            uint256 rDist;
        ...
        function findRecDagVoteNew(address voter, address recipient) public view returns (bool votable, bool voted, uint256 votePos, DagVote memory dagVote){
    ...
    //// Dag internals.
        ///////////// Single vote changes
            ...
            function combinedDagAppendSdist( address voter, address recipient,  uint256 sentDist, uint256 recDist, uint256 weight) internal{
                sentDagVote[voter][sentDagVoteCount[voter]]= DagVote({id: recipient, weight: weight, dist: sentDist, posInOther: recDagVoteCount[recipient]});
                ++(sentDagVoteCount[voter]);
                sentDagVoteTotalWeight[voter] += weight;
                recDagVote[recipient][recDagVoteCount[recipient]]= DagVote({id: voter, weight: weight, dist: recDist, posInOther: sentDagVoteCount[voter]-1});
            ...
            function unsafeReplaceSentDagVoteWithLast(address voter, uint256 sPos) internal {
                ...
                if (sPos!= sentDagVoteCount[voter]-1) {
                    ...
                    sentDagVote[voter][sPos] = DagVote({id: copiedSentDagVote.id, weight: copiedSentDagVote.weight, dist: copiedSentDagVote.dist, posInOther: copiedSentDagVote.posInOther});
            ...
            function unsafeReplaceRecDagVoteWithLast(address recipient, uint256 rPos) public {
                ...
                if (rPos != recDagVoteCount[recipient]-1) {
                    ...
                    recDagVote[recipient][rPos]= DagVote({id:copiedRecDagVote.id, weight:copiedRecDagVote.weight, dist: copiedRecDagVote.dist, posInOther: copiedRecDagVote.posInOther});
        ...
        ///////////// Personal changers
            ...
            function handleDagVoteReplace(
                ...
                for (uint256 i = count; 0 < i ; --i){
                    ...
                    (bool isLocal, uint256 sDist, uint256 rDist) = findDistancesRecNotLower(replacedPositionInTree, sDagVote.id);
                    if (!isLocal) {
                ...
                for (uint256 i = count; 0 < i ; --i){
                    ...
                    (bool isLocal, uint256 sDist, uint256 rDist) = findDistancesRecNotLower(rDagVote.id, replacedPositionInTree);
                    if (!isLocal) {
