...
contract Anthill is IAnthill {
    ...
    function joinTree(address voter, string calldata voterName, address recipient) public {
        ...
        (bool votable, bool voted, uint256 sdist, uint256 depth, , ) = AnthillInner.findSentDagVote(
            dag,
            voter,
            recipient
        );
    ...
    function removeDagVote(address voter, address recipient) public {
        ...
        (, bool voted, uint256 sdist, uint256 depth, uint256 sPos, ) = AnthillInner.findSentDagVote(
            dag,
            voter,
            recipient
        );
            ...
            for (uint256 depth = 1; depth <= dag.MAX_REL_ROOT_DEPTH - dist; depth++) {
                for (uint256 count = 0; count < AnthillInner.readRecDagVoteCount(dag, voter, dist, depth); count++) {
                    ...
                    voterReputation +=
                        ...
                        dag.sentDagVoteTotalWeight[rDagVote.id];
                }
            }
        }
        ...
        for (uint256 dist = 0; dist < dag.MAX_REL_ROOT_DEPTH; dist++) {
            for (uint256 depth = 1; depth <= dag.MAX_REL_ROOT_DEPTH - dist; depth++) {
                for (uint256 count = 0; count < AnthillInner.readRecDagVoteCount(dag, voter, dist, depth); count++) {
                    DagVote memory rDagVote = AnthillInner.readRecDagVote(dag, voter, dist, depth, count);
                    voterReputation +=
                        (dag.reputation[rDagVote.id] * (rDagVote.weight)) /
                        dag.sentDagVoteTotalWeight[rDagVote.id];
                }
            }
        }
    ...
    function moveTreeVote(address voter, address recipient) external {
        ...
        if (positionResult.isLowerOrEqual) {
            dag.handleDagVoteMoveFall(
                voter,
                recipient,
                address(0),
                positionResult.lowerRDist,
                positionResult.lowerDepth
            );
            ...
            dag.handleDagVoteMoveRise(
                voter,
                recipient,
                address(0),
                positionResult.higherDist,
                positionResult.higherDepth
            );
        } else {
    ...
    function _checkPosition(
        address voter,
        address recipient
    ) internal view returns (CheckPositionResult memory result) {
        (address relRoot, ) = AnthillInner.findRelRoot(dag, voter);
        (bool isLowerOrEqual, uint256 lowerSDist, uint256 lowerDepth) = AnthillInner.findSDistDepth(
            dag,
            recipient,
            voter
        );
        ...
        (bool isHigher, uint256 higherRDist, uint256 higherDepthToRelRoot) = AnthillInner.findSDistDepth(
            dag,
            recipient,
            relRoot
        );
    ...
    function unsafeDeleteLastSentDagVoteAtDistDepth(address voter, uint256 sdist, uint256 depth) internal {
        delete dag.sentDagVote[voter][dag.sentDagVoteDistDiff[voter] + sdist][dag.sentDagVoteDepthDiff[voter] + depth][
            dag.readSentDagVoteCount(voter, sdist, depth) - 1
        ];
    }
    ...
    function unsafeDeleteLastRecDagVoteAtDistDepth(address recipient, uint256 rdist, uint256 depth) internal {
        ...
        ][dag.readRecDagVoteCount(recipient, rdist, depth) - 1];
    }
