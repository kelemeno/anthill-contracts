...
library AnthillInner{
    ...
    //// Variable readers 
        ...
        // for sent dag 
            ...
            function readSentDagVoteCount(Dag storage dag, address voter, uint256 sdist, uint256 depth) public view returns(uint256){
                    return dag.sentDagVoteCount[voter][dag.sentDagVoteDistDiff[voter]+sdist][dag.sentDagVoteDepthDiff[voter]+depth];
            }

            function readSentDagVote(Dag storage dag, address voter, uint256 sdist, uint256 depth, uint256 votePos) public view returns( DagVote memory){
        ...
        // for rec Dag votes
            ...
            function readRecDagVoteCount(Dag storage dag, address recipient, uint256 rdist, uint256 depth) public view returns(uint256){
                    return dag.recDagVoteCount[recipient][dag.recDagVoteDistDiff[recipient]+rdist][dag.recDagVoteDepthDiff[recipient]+depth];
            }

            function readRecDagVote(Dag storage dag, address recipient, uint256 rdist, uint256 depth, uint256 votePos) public view returns(DagVote memory){
    ...
    //// Personal tree finder 
        ...
        function findRecTreeVotePos(Dag storage dag, address voter, address recipient) public view returns (bool voted, uint256 votePos) {
    ...
    //// Local tree finders
        ...
        function findRelDepth(Dag storage dag,address voter, address recipient) public view returns (bool isLocal, uint256 relDepth){
        ...
        function findDistAtSameDepth(Dag storage dag,address add1, address add2) public view returns (bool isSameDepth, uint256 distance) {
            if (add1 == add2){
                return (true, 0);
            }
        ...
        // 
        function findSDistDepth(Dag storage dag,address voter, address recipient) public view returns (bool isLocal, uint256 distance, uint256 relDepth){
            if (dag.treeVote[voter] == address(0) || dag.treeVote[recipient] == address(0)) {
                return (false, 0, 0);
            }
    ...
    //// DAG finders
        ...
        function findSentDagVotePosAtDistDepth(Dag storage dag,address voter, address recipient, uint256 sdist,  uint256 depth) public view returns (bool voted, uint256 votePos, DagVote memory vote){
        ...
        function findRecDagVotePosAtDistDepth(Dag storage dag,address voter, address recipient, uint256 rdist, uint256 depth) public view returns (bool voted, uint256 votePos, DagVote memory vote){
        ...
        function findLastSentDagVoteAtDistDepth(Dag storage dag,address voter, uint256 sdist, uint256 depth) public view returns (bool voted, uint256 votePos, DagVote memory vote){
        ...
        function findLastRecDagVoteAtDistDepth(Dag storage dag,address recipient, uint256 rdist, uint256 depth) public view returns (bool voted, uint256 votePos, DagVote memory vote){
        ...
        function findSentDagVote(Dag storage dag,address voter, address recipient) public view returns (bool votable, bool voted, uint256 sdist,  uint256 depth, uint256 votePos, DagVote memory dagVote){ 
            bool isLocal;
        ...
        function findRecDagVote(Dag storage dag,address voter, address recipient) public view returns (bool votable, bool voted, uint256 rdist, uint256 depth, uint256 votePos, DagVote memory dagVote){
                bool isLocal;
                uint256 sdist;
    ...
    //// Dag internals. Core logic. 
        ///////////// Setters      
            ...
            ///////////// Counts 
                ...
                function increaseSentDagVoteCount(Dag storage dag, address voter, uint256 sdist, uint256 depth, uint256 diff) public{
                    dag.sentDagVoteCount[voter][dag.sentDagVoteDistDiff[voter]+sdist][dag.sentDagVoteDepthDiff[voter]+depth] += diff;
                }

                function decreaseSentDagVoteCount(Dag storage dag, address voter, uint256 sdist, uint256 depth, uint256 diff) public{
                    dag.sentDagVoteCount[voter][dag.sentDagVoteDistDiff[voter]+sdist][dag.sentDagVoteDepthDiff[voter]+depth] -= diff;
                }

                function increaseRecDagVoteCount(Dag storage dag, address recipient, uint256 rdist, uint256 depth, uint256 diff) public{
                    dag.recDagVoteCount[recipient][dag.recDagVoteDistDiff[recipient]+rdist][dag.recDagVoteDepthDiff[recipient]+depth] += diff;
                }

                function decreaseRecDagVoteCount(Dag storage dag, address recipient, uint256 rdist, uint256 depth, uint256 diff) public{
                    dag.recDagVoteCount[recipient][dag.recDagVoteDistDiff[recipient]+rdist][dag.recDagVoteDepthDiff[recipient]+depth] -= diff;
                }

            ///////////// Votes

                function setSentDagVote(Dag storage dag, address voter, uint256 sdist, uint256 depth, uint256 sPos, address recipient, uint256 weight, uint256 rPos) public{
                ...
                function setRecDagVote(Dag storage dag, address recipient, uint256 rdist, uint256 depth, uint256 rPos, address voter, uint256 weight, uint256 sPos) public{
        ...
        ///////////// Single vote changes
            ///////////// appending a vote
                ...
                function sentDagAppend(Dag storage dag, address voter, uint256 sdist, uint256 depth, address recipient, uint256 weight, uint256 rPos ) public{
                ...
                function recDagAppend(Dag storage dag, address recipient, uint256 rdist, uint256 depth, address voter, uint256 weight, uint256 sPos ) public{
                ...
                function combinedDagAppendSdist(Dag storage dag, address voter, address recipient,  uint256 sdist, uint256 depth, uint256 weight) public{
            ...
            ///////////// changing position
                ...
                function changePositionSent(Dag storage dag, address voter, uint256 sdist,  uint256 depth, uint256 sPos, uint256 newRPos) public {
                    dag.sentDagVote[voter][dag.sentDagVoteDistDiff[voter]+ sdist][dag.sentDagVoteDepthDiff[voter]+depth][sPos].posInOther = newRPos;
                }

                function changePositionRec(Dag storage dag, address recipient, uint256 rdist, uint256 depth, uint256 rPos, uint256 newSPos) public{
            ...
            ///////////// delete and removal functions
                ...
                function unsafeDeleteLastSentDagVoteAtDistDepth(Dag storage dag, address voter, uint256 sdist, uint256 depth) public {
                ...
                function unsafeDeleteLastRecDagVoteAtDistDepth(Dag storage dag, address recipient, uint256 rdist, uint256 depth) public {
                ...
                function unsafeReplaceSentDagVoteAtDistDepthPosWithLast(Dag storage dag, address voter, uint256 sdist, uint256 depth, uint256 sPos) public {
                ...
                function unsafeReplaceRecDagVoteAtDistDepthPosWithLast(Dag storage dag, address recipient, uint256 rdist, uint256 depth, uint256 rPos) public {
                ...
                function safeRemoveSentDagVoteAtDistDepthPos(Dag storage dag,address voter, uint256 sdist, uint256 depth, uint256 sPos) public {
                    DagVote memory sDagVote = readSentDagVote( dag , voter,sdist, depth, sPos);
                ...
                function safeRemoveRecDagVoteAtDistDepthPos(Dag storage dag, address recipient, uint256 rdist, uint256 depth, uint256 rPos) public {
            ...
            ///////////// change dist and depth
                function changeDistDepthSent(Dag storage dag, address voter, uint256 sdist, uint256 depth, uint256 sPos, uint256 newSDist, uint256 newDepth, address recipient, uint256 rPos, uint256 weight) public{
                ...
                function changeDistDepthRec(Dag storage dag, address recipient, uint256 rdist, uint256 depth, uint256 rPos, uint256 newRDist, uint256 newDepth, address voter, uint256 sPos, uint256 weight) public{
                    // here it is ok to use unsafe, as the the vote is moved, not removed
                    unsafeReplaceRecDagVoteAtDistDepthPosWithLast( dag , recipient, rdist, depth, rPos);                
        ...
        ///////////// Cell removal and handler functions 
            ...
            ///////////// dist depth on opposite 
                function changeDistDepthFromSentCellOnOp(Dag storage dag, address voter, uint256 sdist, uint256 depth, uint256 oldSDist, uint256 oldDepth) public {
                    for (uint256 i = 0; i < readSentDagVoteCount(dag, voter, sdist, depth); i++) {
                        DagVote memory sDagVote = readSentDagVote(dag, voter, sdist, depth, i);
                ...
                function changeDistDepthFromRecCellOnOp(Dag storage dag, address recipient, uint256 rdist, uint256 depth, uint256 oldRDist, uint256 oldDepth) public {
                    for (uint256 i = 0; i <  readRecDagVoteCount(dag, recipient, rdist, depth); i++) {
            ...
            ///////////// move cell
                ...
                function moveSentDagVoteCell(Dag storage dag,address voter, uint256 sdist, uint256 depth, uint256 newSDist, uint256 newDepth) public {
                ...
                function moveRecDagVoteCell(Dag storage dag,address recipient, uint256 rdist, uint256 depth, uint256 newRDist, uint256 newDepth) public {
        ...
        ///////////// Line  remover and sorter functions
            ...
            ///////////// Sort Cell into line
                function sortSentDagVoteCell(Dag storage dag, address voter, uint256 sdist, uint256 depth, address anscestorAtDepth) public {
                    for (uint256 i = readSentDagVoteCount( dag , voter, sdist, depth); 0 < i ; i--) {
                        DagVote memory sDagVote = readSentDagVote( dag , voter, sdist, depth, i-1);
                ...
                function sortRecDagVoteCell(Dag storage dag, address recipient, uint256 rdist, uint256 depth,  address newTreeVote) public {
                    for (uint256 i = readRecDagVoteCount( dag , recipient, rdist, depth); 0 < i; i--) {
                        DagVote memory rDagVote = readRecDagVote( dag , recipient, rdist, depth, i-1);
        ...
        ///////////// Area/whole triangle changers
            ...
            ///////////// Combined dag Square vote handler for rising falling, a certain depth, with passing the new recipient in for selction    
                ...
                function handleDagVoteMoveRise(Dag storage dag, address voter, address recipient, address replaced, uint256 moveDist, uint256 depthToRec ) public {
                    // sent 
                ...
                function handleDagVoteMoveFall(Dag storage dag, address voter, address recipient, address replaced, uint256 moveDist, uint256 depthToRec) public {
                    // sent 
